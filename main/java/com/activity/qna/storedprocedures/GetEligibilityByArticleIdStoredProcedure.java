...
public class GetEligibilityByArticleIdStoredProcedure extends OracleStoredProcedure {

	private static final String PARAM_ACTIVITY_ID = "PV_ID";
	private static final String PARAM_ELIG_RESULT_SET = "PV_RESULT";
	
	public GetEligibilityByArticleIdStoredProcedure(DataSource dataSource, String storedProcedureName, ResultSetExtractor eligibilityResultSetExtractor ) {

		this.setDataSource(dataSource);
		this.setSql(storedProcedureName);

		declareParameter(new SqlParameter(PARAM_ACTIVITY_ID, Types.INTEGER));
		declareOutCursorParameter(GetEligibilityByArticleIdStoredProcedure.PARAM_ELIG_RESULT_SET, eligibilityResultSetExtractor);		
	...
	public HashMap execute(int activityId) throws QNADataAccessException {
		...
		try {
			...
			HashMap eligibilityMap = (HashMap) map.get(GetEligibilityByArticleIdStoredProcedure.PARAM_ELIG_RESULT_SET);
